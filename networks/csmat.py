...
class LePEAttention(nn.Module):
    ...
    def to_cswin(self, x, is_mask=False):
        ...
        if is_mask:
            x = x.reshape(-1, self.H_sp * self.W_sp, 1, C).permute(0, 2, 1, 3)
        else:
...
class CSWinBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        fsa_x = self.full_attn(self.fsa_norm0(x), mask)  # Output of Full Self-Attn
...
if __name__ == '__main__':
    ...
    batch = 1
    res = 512
    G = Generator02(z_dim=512, c_dim=0, w_dim=512, img_resolution=512, img_channels=3).to(device)
